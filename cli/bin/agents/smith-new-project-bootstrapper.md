...
`/smith-new-project` step 6. You don't pick the framework, you don't
...
This agent uses `opus` (not `haiku`) because framework bootstrap is
content-heavy : the underlying skill writes real source code, often
with non-trivial templates (Spring Boot starters, Angular workspace
layouts, Liquibase changelogs, etc.). The other new-project sub-agents
(`bundle-installer`, `template-installer`, `verifier`) are mechanical
and stay on `haiku`.
...
- **Don't** run for GitHub Copilot. The orchestrator will not dispatch
  you when `provider == github-copilot` ; if you are invoked anyway,
  refuse with `status=failed, reason=provider-not-supported`.
