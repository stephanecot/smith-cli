---
name: smith-help
description: Reference for the Smith CLI consumer surface. Use this skill whenever the developer asks how Smith works, what `/smith-new-project` / `/smith-init` / `/smith-generate-docs` / `/smith-bundle-list` / `/dashboard-ai` do, what `.smith/architecture.json` + `.smith/config.json` + `.smith/smith.yaml` + `.smith/report.md` + `AGENTS.md` contain, what bundles + templates are, or how to get started. Covers ONLY the consumer-facing skills shipped under `bin/skills/` — CLI-maintainer commands (`/smith-bundle-add`, `/smith-provider-add`, `/smith-template-add`, `/smith-build`, …) are out of scope. Auto-load on any question about Smith, smith-cli, smith bundles, smith templates, or `/smith-*` slash commands.
...
2. **Release-tree model.** Every artefact is pre-built by
   `/smith-build` into `releases/<provider>/`. The consumer-side
