...
- `templates/index.yaml` must list at least one entry for the
  requested `<framework>`.
- The resolved `<provider>` MUST appear in
  `templates/framework/<framework>/<version>/config.yaml` `providers:`.
...
1. Read `templates/index.yaml`. Filter entries with the requested
   `framework`.
...
2. **Load the template config** at
   `templates/framework/<framework>/<version>/config.yaml`. Confirm
...
3. **Load the provider's format spec** :
   ...
   - `providers/<provider>/format-skill.yaml` →
...
5. **Record the install in `<consumer>/.smith/config.json`** :
   ...
   - `version` comes from `templates/framework/<framework>/<version>/config.yaml`
...
- Don't write to `templates/` — read-only on the catalogue.
