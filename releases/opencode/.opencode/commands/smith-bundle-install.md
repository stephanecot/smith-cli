...
description: Installs a bundle from bundles/<name>/ into the consumer project. Assembles each skill at install time by composing the frontmatter from bundles/<name>/skills/<slug>/metadata.yml + <provider>.yml, validated against providers/<provider>/format-skill.yaml, then prepending it to the skill body. Hooks under bundles/<name>/hooks/<provider>/ are merged into the consumer's provider config; sidecar scripts in the same folder are copied to the provider's scripts directory. Trigger with `/smith-bundle-install --name <bundle> --ia <provider>`. Requires /smith-init to have run.
---
...
- `--name <bundle>` must be a key listed in `bundles/index.yaml`.
- `--ia <provider>` must be in the bundle's `config.yaml` `providers:`
  list AND a known Smith provider (`providers/<provider>/` exists).
...
/smith-bundle-install --name ia-stats --ia claude-code --consumer-dir samples/angular-sample
```
...
4. **Walk `bundles/<bundle>/skills/`** and assemble one skill at a
...
- **Don't modify `bundles/index.yaml`** (that's
